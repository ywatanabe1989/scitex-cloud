...
# File: ./deployment/nginx/maintenance/nginx_check_dns.sh
#
# SciTeX Cloud - DNS and SSL Check for Nginx
...
NGINX_CONF="/etc/nginx/sites-available/scitex.conf"
NGINX_ENABLED="/etc/nginx/sites-enabled/scitex.conf"
...
if [ "$DNS_OK" = true ] && [ "$SSL_OK" = true ] && [ "$NGINX_OK" = true ]; then
    ...
    echo "  2. Add A record: $DOMAIN → $ACTUAL_IP"
