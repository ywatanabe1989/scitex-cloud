 ...
 */
function toggleAbstractMode(button, paperId, mode) {
    console.log('[Search UI] Toggling abstract mode:', paperId, 'to', mode);
    const abstractPreview = document.querySelector(`.abstract-preview[data-abstract-id="${paperId}"]`);
    if (!abstractPreview) {
...
function openAllPaperUrls() {
    ...
    if (urls.length > 5) {
        const confirmed = confirm(`This will open ${urls.length} new tabs. Your browser may block some of them. Continue?`);
        if (!confirmed) {
