...
function initQueueManagement(config = {}) {
    ...
    const { resourceStatusUrl = '/scholar/api/bibtex/resource-status/', pollInterval = 2000 } = config;
...
function updateResourceMonitor(resourceStatusUrl) {
    fetch(resourceStatusUrl)
        ...
        const refreshTimeEl = document.getElementById('resourceRefreshTime');
        if (refreshTimeEl) {
...
function updateSystemStats(system) {
    ...
    if (cpuElement)
        cpuElement.textContent = system.cpu_percent;
    if (memoryElement)
        memoryElement.textContent = system.memory_percent;
}
/**
...
function updateActiveJobs(jobs) {
    ...
    if (!activeJobsList || !activeJobsContent)
        return;
    // Update count
    if (activeJobsCount)
...
function updateQueuedJobs(jobs) {
    ...
    if (!queuedJobsList || !queuedJobsContent)
        return;
    // Update count
    if (queuedJobsCount)
