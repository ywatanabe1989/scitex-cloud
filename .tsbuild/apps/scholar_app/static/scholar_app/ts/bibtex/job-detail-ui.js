 ...
 */
console.log("[DEBUG] apps/scholar_app/static/scholar_app/ts/bibtex/job-detail-ui.ts loaded");
/**
 ...
 */
async function copyLogToClipboard(button, logElement) {
    ...
    try {
        ...
        // Show error feedback
        button.innerHTML = '<i class="fas fa-exclamation-triangle"></i> Select text manually';
 ...
 */
function toggleLogSize(button, logElement, expanded) {
    console.log('[Job Detail UI] Toggling log size, currently expanded:', expanded);
 ...
 * @param {HTMLPreElement} logElement - The log content element
 */
function handleLogKeyboardShortcut(event, logElement) {
 ...
 */
function updateElapsedTime(startedAt, elapsedTimeEl) {
 ...
 */
function pollJobStatus(jobId, jobStatus, logElement, attempts = 0, maxAttempts = 90) {
    ...
    fetch(`/scholar/api/bibtex/job/${jobId}/status/`)
        ...
        .then(data => {
        console.log(`[POLL-SCHOLAR] Status: ${data.status}, Has log: ${!!data.log}, Log length: ${data.log ? data.log.length : 0}`);
        ...
        // Update progress
        const progressBar = document.querySelector('[style*="background: var(--scitex-color-03)"]');
        if (progressBar && data.progress_percentage !== undefined) {
            progressBar.style.width = `${data.progress_percentage}%`;
            console.log(`[POLL-SCHOLAR] Progress: ${data.progress_percentage}%`);
        }
        // Update paper counts
        const paperCount = document.querySelector('[style*="color: var(--color-fg-muted)"]');
        ...
        if (data.status === 'completed' || data.status === 'failed') {
            ...
            // Continue polling
            setTimeout(() => pollJobStatus(jobId, jobStatus, logElement, attempts + 1, maxAttempts), 2000);
        }
    })
        .catch(error => {
        console.error('[POLL-SCHOLAR] Error polling job status:', error.message || error);
        ...
        setTimeout(() => pollJobStatus(jobId, jobStatus, logElement, attempts + 1, maxAttempts), backoffDelay);
    });
}
/**
...
function initJobDetailUI() {
    ...
    if (!jobContainer) {
        console.log('[Job Detail UI] No job container found, skipping initialization');
        return;
    }
    ...
    const elements = {
        ...
        toggleLogSizeBtn: document.getElementById('toggle-log-size'),
    ...
    if (elements.toggleLogSizeBtn && elements.processingLog) {
        elements.toggleLogSizeBtn.addEventListener('click', () => {
            logExpanded = toggleLogSize(elements.toggleLogSizeBtn, elements.processingLog, logExpanded);
        });
    }
...
//# sourceMappingURL=job-detail-ui.js.map