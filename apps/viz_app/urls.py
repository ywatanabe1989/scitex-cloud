...
urlpatterns = [
    ...
    path(
        "view/<uuid:pk>/<str:share_token>/",
        views.view_visualization,
        name="view_shared_visualization",
    ),
    path("embed/<uuid:pk>/", views.embedded_view, name="embedded_view"),
    path(
        "embed/<uuid:pk>/<str:embed_token>/",
        views.embedded_view,
        name="embedded_view_token",
    ),
    ...
    # AJAX endpoints
    path(
        "api/update-data/<uuid:pk>/",
        views.update_visualization_data,
        name="update_visualization_data",
    ),
    path("api/add-comment/<uuid:pk>/", views.add_comment, name="add_comment"),
    # New API endpoints for interactive viz interface
    path(
        "api/visualizations/",
        api_views.VisualizationListAPI.as_view(),
        name="api_visualizations",
    ),
    path(
        "api/visualizations/<uuid:viz_id>/",
        api_views.VisualizationDetailAPI.as_view(),
        name="api_visualization_detail",
    ),
    path(
        "api/data-sources/", api_views.DataSourceAPI.as_view(), name="api_data_sources"
    ),
    path(
        "api/visualization-types/",
        api_views.VisualizationTypesAPI.as_view(),
        name="api_visualization_types",
    ),
    path(
        "api/color-schemes/",
        api_views.ColorSchemesAPI.as_view(),
        name="api_color_schemes",
    ),
    ...
    path(
        "api/export-status/<uuid:job_id>/",
        api_views.export_job_status,
        name="api_export_status",
    ),
    path("api/sample-data/", api_views.generate_sample_data, name="api_sample_data"),
    # Code integration endpoint
    path(
        "api/code-sources/",
        api_views.CodeDataSourceAPI.as_view(),
        name="api_code_data_sources",
    ),
]
