...
def viz_dashboard(request):
    ...
    visualizations = (
        Visualization.objects.filter(owner=request.user)
        .select_related("visualization_type", "data_source")
        .order_by("-updated_at")
    )

    # Get recent analytics
    recent_analytics = VisualizationAnalytics.objects.filter(
        visualization__owner=request.user, date__gte=timezone.now() - timedelta(days=30)
    ).aggregate(
    ...
    # Get dashboards
    dashboards = (
        Dashboard.objects.filter(owner=request.user)
        .annotate(viz_count=Count("visualizations"))
        .order_by("-updated_at")[:5]
    )

    # Get shared visualizations
    shared_with_me = (
        VisualizationShare.objects.filter(
            shared_with=request.user, expires_at__gte=timezone.now()
        )
        .select_related("visualization", "shared_by")
        ...
        try:
            ...
            with transaction.atomic():
                visualization = Visualization.objects.create(
                    ...
                    configuration=viz_type.default_config.copy()
                    if viz_type.default_config
                    else {},
                )
                ...
                messages.success(
                    request, f"Visualization '{title}' created successfully!"
                )
...
def edit_visualization(request, pk):
    ...
    if visualization.owner != request.user:
        ...
        if not share:
            raise PermissionDenied(
                "You don't have permission to edit this visualization"
            )
    ...
    if request.method == "POST":
        try:
            ...
            visualization.description = request.POST.get(
            ...
            if chart_config:
                ...
                chart_config.legend_position = request.POST.get(
...
def view_visualization(request, pk, share_token=None):
    ...
    analytics, created = VisualizationAnalytics.objects.get_or_create(
        visualization=visualization, date=today, defaults={"views": 1}
    )
    if not created:
    ...
    if can_view:
        comments = (
            VisualizationComment.objects.filter(
                visualization=visualization, is_deleted=False, parent__isnull=True
            )
        ...
        try:
            ...
            for idx, viz_id in enumerate(viz_ids):
                visualization = get_object_or_404(
                    Visualization, pk=viz_id, owner=request.user
                )
                DashboardVisualization.objects.create(
                    dashboard=dashboard, visualization=visualization, order=idx
                )
        ...
        if action == "create":
            try:
                ...
                if source_type == "file":
                    ...
                    data_source.refresh_interval = int(
                        request.POST.get("refresh_interval", 3600)
                    )
                    data_source.save()
                    ...
                    if code_job_id:
                        ...
                        try:
                            ...
                            data_source = bridge.create_data_source_from_code_job(
                                code_job
                            )
                            if data_source:
                                messages.success(
                                    request,
                                    f"Code execution data source '{data_source.name}' created successfully!",
                                )
                            else:
                                messages.error(
                                    request,
                                    "Failed to create data source from code execution",
                                )
                        except CodeExecutionJob.DoesNotExist:
                            messages.error(
                                request, "Code execution job not found or not completed"
                            )
                    else:
                        messages.error(
                            request,
                            "Code job ID is required for code execution data source",
                        )
                else:
                    messages.success(
                        request, f"Data source '{name}' created successfully!"
                    )
            ...
            try:
                data_source = get_object_or_404(
                    DataSource, pk=data_source_id, owner=request.user
                )
                data_source.delete()
...
def export_visualization(request, pk):
    ...
    if visualization.owner != request.user and not visualization.is_public:
        ...
        if not share:
            raise PermissionDenied(
                "You don't have permission to export this visualization"
            )
    ...
    try:
        ...
        if export_format == "json":
            ...
            response["Content-Disposition"] = (
                f'attachment; filename="{visualization.title}.json"'
            )
            ...
            analytics, _ = VisualizationAnalytics.objects.get_or_create(
                visualization=visualization, date=timezone.now().date()
            )
            ...
            response["Content-Disposition"] = (
                f'attachment; filename="{visualization.title}.csv"'
            )

            if visualization.data and isinstance(visualization.data, list):
                writer = csv.DictWriter(
                    response, fieldnames=visualization.data[0].keys()
                )
                writer.writeheader()
...
def share_visualization(request, pk):
    ...
    if request.method == "POST":
        try:
            ...
            if action == "create_share":
                ...
                share = get_object_or_404(
                    VisualizationShare, pk=share_id, visualization=visualization
                )
                share.delete()
    ...
    shares = (
        VisualizationShare.objects.filter(visualization=visualization)
    ...
    context = {
        "visualization": visualization,
        "shares": shares,
        "share_url": request.build_absolute_uri(f"/viz/view/{visualization.pk}/"),
    }
...
def update_visualization_data(request, pk):
    ...
    try:
        ...
        visualization.configuration = data.get(
            "configuration", visualization.configuration
        )
        visualization.save()
    ...
    return JsonResponse(
        {
            ...
            "download_url": f"/viz/download/{export_job.pk}/"
