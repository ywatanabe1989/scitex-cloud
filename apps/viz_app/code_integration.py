...
class CodeVizBridge:
    ...
    def create_data_source_from_code_job(self, code_job: CodeExecutionJob) -> Optional[DataSource]:
        ...
        try:
            ...
            logger.info(f"Created data source {data_source.id} from code job {code_job.job_id}")
            return data_source
            
        except Exception as e:
            logger.error(f"Error creating data source from code job {code_job.job_id}: {e}")
            return None
    ...
    def _extract_data_schema(self, code_job: CodeExecutionJob) -> Dict[str, Any]:
        ...
        try:
            ...
            for plot_file in code_job.plot_files:
                plot_path = self.media_root / 'visualizations' / str(code_job.user.id) / plot_file
                if plot_path.exists():
                    schema['plots'].append({
                        'filename': plot_file,
                        'path': str(plot_path),
            ...
            for output_file in code_job.output_files:
                if isinstance(output_file, dict):
                    ...
                    output_path = self.media_root / 'code_outputs' / str(code_job.user.id) / output_file
            ...
            logger.warning(f"Error extracting schema from code job {code_job.job_id}: {e}")
    ...
    def create_visualization_from_code_job(self, code_job: CodeExecutionJob, 
                                         plot_filename: str = None) -> Optional[Visualization]:
        ...
        try:
            ...
            if target_plot not in code_job.plot_files:
                logger.error(f"Plot file {target_plot} not found in code job {code_job.job_id}")
                return None
            ...
            if not viz_type:
                logger.error(f"Could not determine visualization type for {target_plot}")
                return None
            ...
            visualization = Visualization.objects.create(
                ...
                configuration={
                    ...
                    'code_job_id': str(code_job.job_id),
                    'original_code_preview': code_job.source_code[:200] + "..." if len(code_job.source_code) > 200 else code_job.source_code
                },
                status='published',
                is_public=False,
            ...
            logger.info(f"Created visualization {visualization.id} from code job {code_job.job_id}")
            return visualization
            
        except Exception as e:
            logger.error(f"Error creating visualization from code job {code_job.job_id}: {e}")
            return None
    
    def _determine_visualization_type(self, plot_filename: str) -> Optional[VisualizationType]:
            ...
            if data_source:
                ...
                for plot_file in code_job.plot_files:
                    ...
                    if viz:
                        ...
                        result['errors'].append(f"Failed to create visualization for {plot_file}")
            else:
