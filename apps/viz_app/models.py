...
class ColorScheme(models.Model):
    ...
    owner = models.ForeignKey(
        User, on_delete=models.CASCADE, related_name="color_schemes"
    )
...
class DataSource(models.Model):
    ...
    connection_config = (
        models.JSONField()
    )  # Connection details, encrypted sensitive data
    schema = models.JSONField(null=True, blank=True)  # Data schema/structure
    refresh_interval = models.IntegerField(
        null=True, blank=True, help_text="Refresh interval in seconds"
    )
...
class VisualizationTemplate(models.Model):
    ...
    name = models.CharField(max_length=200)
    owner = models.ForeignKey(
        User, on_delete=models.CASCADE, related_name="viz_templates"
    )
...
class Visualization(models.Model):
    ...
    owner = models.ForeignKey(
        User, on_delete=models.CASCADE, related_name="visualizations"
    )
    ...
    template = models.ForeignKey(
        VisualizationTemplate, on_delete=models.SET_NULL, null=True, blank=True
    )

    # Visualization data and configuration
    data = models.JSONField(
        null=True, blank=True
    )  # Embedded data if not using DataSource
    ...
    color_scheme = models.ForeignKey(
        ColorScheme, on_delete=models.SET_NULL, null=True, blank=True
    )
...
class ChartConfiguration(models.Model):
    ...
    visualization = models.OneToOneField(
        Visualization, on_delete=models.CASCADE, related_name="chart_config"
    )
    ...
    x_axis_type = models.CharField(
        max_length=20, default="linear"
    )  # linear, log, category, datetime
...
class InteractiveElement(models.Model):
    ...
    visualization = models.ForeignKey(
        Visualization, on_delete=models.CASCADE, related_name="interactive_elements"
    )
    ...
    target_data = models.CharField(
        max_length=200, blank=True
    )  # Data field this element affects
...
class Dashboard(models.Model):
    ...
    visualizations = models.ManyToManyField(
        Visualization, through="DashboardVisualization"
    )
...
class VisualizationShare(models.Model):
    ...
    visualization = models.ForeignKey(
        Visualization, on_delete=models.CASCADE, related_name="shares"
    )
    shared_by = models.ForeignKey(
        User, on_delete=models.CASCADE, related_name="shared_visualizations"
    )
    shared_with = models.ForeignKey(
        User,
        on_delete=models.CASCADE,
        related_name="received_visualizations",
        null=True,
        blank=True,
    )
...
class ExportJob(models.Model):
    ...
    visualization = models.ForeignKey(
        Visualization, on_delete=models.CASCADE, related_name="export_jobs"
    )
    user = models.ForeignKey(User, on_delete=models.CASCADE, related_name="export_jobs")
    format = models.CharField(max_length=10, choices=FORMAT_CHOICES)
    options = models.JSONField(
        default=dict
    )  # Format-specific options (DPI, size, etc.)
...
class VisualizationComment(models.Model):
    ...
    annotation_data = models.JSONField(
        null=True, blank=True
    )  # Coordinates, element references
...
class VisualizationAnalytics(models.Model):
    ...
    visualization = models.ForeignKey(
        Visualization, on_delete=models.CASCADE, related_name="analytics"
    )
