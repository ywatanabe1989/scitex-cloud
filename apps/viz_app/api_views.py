        ...
        try:
            ...
            for viz in paginated_visualizations:
                visualizations_data.append({
                    ...
                    'published_at': viz.published_at.isoformat() if viz.published_at else None,
                    'data_source': viz.data_source.name if viz.data_source else None,
                    'color_scheme': viz.color_scheme.name if viz.color_scheme else None,
                    'tags': viz.tags,
                    'version': viz.version
                })
...
class VisualizationDetailAPI(VizAPIView):
    ...
    def get(self, request, viz_id):
        ...
        try:
            visualization = self.get_user_visualizations().select_related(
            ...
            return JsonResponse({
                ...
                'visualization': {
                    ...
                    } if hasattr(visualization, 'chart_config') else None
                }
            })
...
class VisualizationTypesAPI(View):
    ...
    def get(self, request):
        ...
        try:
            viz_types = VisualizationType.objects.filter(is_active=True).order_by('category', 'display_name')
                ...
                if export_format == 'json':
                    ...
                    if visualization.data and isinstance(visualization.data, dict):
                        ...
                        if 'x' in visualization.data and 'y' in visualization.data:
                            ...
                            writer.writerow(['x', 'y'] + (['z'] if 'z' in visualization.data else []))
...
def upload_data(request):
    ...
    try:
        ...
        if file_extension == '.csv':
            ...
            return JsonResponse({'status': 'error', 'message': 'File parsing not implemented for this type'}, status=400)
...
def generate_sample_data(request):
    ...
    try:
        ...
        if data_type == 'scatter':
            ...
            z = [math.sin(x[i]) * math.cos(y[i]) + random.uniform(-0.5, 0.5) for i in range(size)]
            ...
            dates = [(start_date + datetime.timedelta(days=i)).isoformat() for i in range(size)]
            values = [50 + 10 * math.sin(i * 0.1) + random.uniform(-5, 5) for i in range(size)]
        ...
        try:
            ...
            if result.get('data_source_created'):
                return JsonResponse({
                    ...
                    'visualizations_created': result.get('visualizations_created', 0),
