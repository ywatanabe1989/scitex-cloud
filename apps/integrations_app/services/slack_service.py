...
class SlackService:
    ...
    def create_webhook(self, webhook_url, channel='', enabled_events=None, project_ids=None):
        """
        ...
        if enabled_events is None:
            enabled_events = ['project_created', 'manuscript_updated', 'analysis_completed']
        ...
        self._log_activity(connection, 'connect', f'Slack webhook created for channel: {channel or "default"}')
...
def notify_project_created(project):
    ...
    return service.send_notification('project_created', {
        'project_id': project.id,
        'project_name': project.name,
        'user': project.owner.username,
        'url': project.get_absolute_url() if hasattr(project, 'get_absolute_url') else '',
    })
...
def notify_manuscript_updated(manuscript, project):
    ...
    service = SlackService(project.owner)
    return service.send_notification('manuscript_updated', {
        'project_id': project.id,
        'project_name': project.name,
...
def notify_analysis_completed(project, analysis_type=''):
    ...
    service = SlackService(project.owner)
    return service.send_notification('analysis_completed', {
        'project_id': project.id,
        'project_name': project.name,
        'user': project.owner.username,
