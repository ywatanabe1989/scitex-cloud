...
def export_project_bib(request, project_id):
    ...
    try:
        ...
        response['Content-Disposition'] = f'attachment; filename="{project.get_filesystem_safe_name()}_references.bib"'
...
def slack_test(request, webhook_id):
    ...
    if result['success']:
        ...
        messages.error(request, f'Failed to send test notification: {result.get("error", "Unknown error")}')
...
def slack_delete(request, webhook_id):
    ...
    if result['success']:
        ...
        messages.error(request, f'Failed to delete webhook: {result.get("error", "Unknown error")}')
