...
def star_repository(request, username, slug):
    ...
    if RepositoryStar.is_starred(request.user, project):
        return JsonResponse({'success': False, 'error': 'Already starred this repository'}, status=400)
...
def unstar_repository(request, username, slug):
    ...
    if deleted_count == 0:
        return JsonResponse({'success': False, 'error': 'Not starred this repository'}, status=400)
...
def followers_list(request, username):
    ...
    followers_data = [{
        ...
        'avatar_url': f.follower.profile.avatar.url if hasattr(f.follower, 'profile') and f.follower.profile.avatar else None,
...
def following_list(request, username):
    ...
    following_data = [{
        ...
        'avatar_url': f.following.profile.avatar.url if hasattr(f.following, 'profile') and f.following.profile.avatar else None,
        'followed_at': f.created_at.isoformat()
    } for f in following]
...
def stargazers_list(request, username, slug):
    ...
    stargazers_data = [{
        ...
        'avatar_url': s.user.profile.avatar.url if hasattr(s.user, 'profile') and s.user.profile.avatar else None,
