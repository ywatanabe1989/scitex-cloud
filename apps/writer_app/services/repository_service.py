...
class ManuscriptRepositoryIntegrator:
    ...
    def create_supplementary_dataset(
        ...
        if not self.repository_connection:
            logger.warning(
                f"No repository connection available for user {self.manuscript.owner.username}"
            )
            return None
        ...
        try:
            with transaction.atomic():
                # Create dataset
                dataset_title = (
                    title or f"Supplementary Data for: {self.manuscript.title}"
                )
                dataset_description = (
                    description or self._generate_dataset_description()
                )
                ...
                logger.info(
                    f"Created supplementary dataset {dataset.id} for manuscript {self.manuscript.slug}"
                )
                return dataset
    ...
    def create_replication_dataset(
        ...
        try:
            with transaction.atomic():
                ...
                self._add_replication_materials_to_dataset(
                    dataset, code_outputs, analysis_data
                )
                ...
                logger.info(
                    f"Created replication dataset {dataset.id} for manuscript {self.manuscript.slug}"
                )
                return dataset
        ...
        try:
            with transaction.atomic():
                dataset = Dataset.objects.create(
                    ...
                    description=self._generate_arxiv_dataset_description(
                        arxiv_submission
                    ),
                    dataset_type="supplementary",
                    owner=self.manuscript.owner,
    ...
    def _generate_arxiv_dataset_description(
        self, arxiv_submission: ArxivSubmission
    ) -> str:
    ...
    def _generate_keywords(self) -> str:
        ...
        if self.manuscript.keywords:
            keywords.extend(
                [k.strip() for k in self.manuscript.keywords.split(",") if k.strip()]
            )
            ...
            try:
                ...
                pdf_file = DatasetFile.objects.create(
                    ...
                    local_file=ContentFile(
                        pdf_content, name=f"{self.manuscript.slug}.pdf"
                    ),
                )
        ...
        if self.manuscript.is_modular:
            ...
            if self.manuscript.content:
                ...
                latex_file = DatasetFile.objects.create(
                    ...
                    local_file=ContentFile(
                        latex_content, name=f"{self.manuscript.slug}.tex"
                    ),
                )
    ...
    def _add_replication_materials_to_dataset(
        self, dataset: Dataset, code_outputs: List = None, analysis_data: List = None
    ):
        ...
        if self.manuscript.project:
            ...
            code_jobs = CodeExecutionJob.objects.filter(
                user=self.manuscript.owner, status="completed"
            )
            ...
            for job in code_jobs[:5]:  # Limit to recent jobs
                ...
                job_file = DatasetFile.objects.create(
                    ...
                    local_file=ContentFile(
                        job_content, name=f"code_job_{job.job_id}.json"
                    ),
                )
    ...
    def _add_arxiv_files_to_dataset(
        self, dataset: Dataset, arxiv_submission: ArxivSubmission
    ):
    ...
    def _add_manuscript_metadata_to_dataset(self, dataset: Dataset):
        ...
        metadata = {
            ...
            "abstract": self.manuscript.abstract,
            "keywords": self.manuscript.keywords,
            "owner": self.manuscript.owner.get_full_name()
            or self.manuscript.owner.username,
            "collaborators": [
            ...
            "submission_deadline": self.manuscript.submission_deadline.isoformat()
            if self.manuscript.submission_deadline
            else None,
            ...
            "created_at": self.manuscript.created_at.isoformat(),
            "updated_at": self.manuscript.updated_at.isoformat(),
            "last_compiled": self.manuscript.last_compiled.isoformat()
            if self.manuscript.last_compiled
            else None,
        }
    ...
    def _generate_replication_readme(self) -> str:
        ...
        readme += f"### Citation\n\n"
        readme += (
            f"If you use these materials, please cite the original manuscript:\n\n"
        )
        readme += f"```\n"
