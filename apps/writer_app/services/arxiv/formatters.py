...
class ArxivLatexFormatter:
    ...
    def format_for_arxiv(self, manuscript: Manuscript, document_class: str = 'article') -> str:
        """
    ...
    def _format_modular_manuscript(self, manuscript: Manuscript, document_class: str) -> str:
    ...
    def _format_standard_manuscript(self, manuscript: Manuscript, document_class: str) -> str:
    ...
    def _generate_default_main_tex(self, manuscript: Manuscript, document_class: str) -> str:
        """Generate default main.tex structure."""
        template = self.arxiv_class_options.get(document_class, self.arxiv_class_options['article'])
...
"""
    ...
    def _replace_input_commands(self, main_content: str, sections: Dict[str, str]) -> str:
    ...
    def _generate_complete_latex(self, manuscript: Manuscript, document_class: str) -> str:
    ...
    def _remove_problematic_packages(self, content: str) -> str:
        ...
        for problematic, replacement in self.problematic_packages.items():
            ...
            if replacement:
                ...
                content = re.sub(pattern, f'% Removed unsupported package: {problematic}', content)
    ...
    def _fix_common_latex_issues(self, content: str) -> str:
        ...
        content = re.sub(r'\\includegraphics\{[^}]*[/\\]([^/\\}]+)\}', r'\\includegraphics{\1}', content)
        
        # Ensure UTF-8 encoding
        if '\\usepackage[utf8]{inputenc}' not in content and '\\documentclass' in content:
    ...
    def _validate_packages(self, content: str) -> str:
        ...
        for package in packages:
            if package not in self.approved_packages:
                warnings.append(f"Warning: Package '{package}' may not be supported by arXiv")
        
        if warnings:
            warning_comment = '% arXiv Package Warnings:\n% ' + '\n% '.join(warnings) + '\n\n'
        ...
        with zipfile.ZipFile(package_path, 'w', zipfile.ZIP_DEFLATED) as zipf:
            ...
            if figures_dir.exists():
                for figure_file in figures_dir.iterdir():
                    if figure_file.is_file() and figure_file.suffix.lower() in self.allowed_extensions:
            ...
            for file_path in work_dir.iterdir():
                if (file_path.is_file() and 
                    ...
                    file_path.name not in ['main.tex', 'references.bib']):
                    zipf.write(file_path, file_path.name)
        
        # Check file size
        if package_path.stat().st_size > self.max_file_size:
            raise ValueError(f"Submission package exceeds {self.max_file_size / (1024*1024):.1f}MB limit")
...
class ArxivComplianceChecker:
    ...
    def check_compliance(self, manuscript: Manuscript, latex_content: str) -> Dict[str, any]:
        """
        Check manuscript compliance with arXiv requirements.
    ...
    def _check_abstract(self, abstract: str) -> Dict:
        ...
        if not abstract or not abstract.strip():
            ...
            if len(abstract) > self.max_abstract_length:
                ...
                warnings.append(f"Abstract is shorter than recommended minimum of {self.min_abstract_length} characters")
    ...
    def _check_categories(self, manuscript: Manuscript) -> Dict:
        ...
        if not hasattr(manuscript, 'arxiv_submissions') or not manuscript.arxiv_submissions.exists():
