 ...
 */
function setupSectionManagementButtons(config, state, sectionsManager, editor) {
    ...
    if (addSectionBtn) {
        addSectionBtn.addEventListener('click', () => {
            const modal = new window.bootstrap.Modal(document.getElementById('add-section-modal'));
            modal.show();
        });
    }
    // Confirm Add Section
    ...
    if (confirmAddBtn) {
        confirmAddBtn.addEventListener('click', async () => {
            ...
            try {
                ...
                const response = await fetch(`/writer/api/project/${config.projectId}/section/create/`, {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json',
                        'X-CSRFToken': getCsrfToken()
    ...
    if (deleteSectionBtn) {
        deleteSectionBtn.addEventListener('click', () => {
            ...
            const modal = new window.bootstrap.Modal(document.getElementById('delete-section-modal'));
            modal.show();
        });
    }
    ...
    if (confirmDeleteBtn) {
        confirmDeleteBtn.addEventListener('click', async () => {
            ...
            if (!currentSection)
                return;
            try {
                const response = await fetch(`/writer/api/project/${config.projectId}/section/${encodeURIComponent(currentSection)}/delete/`, {
                    method: 'DELETE',
                    headers: {
                        'Content-Type': 'application/json',
                        'X-CSRFToken': getCsrfToken()
    ...
    if (toggleIncludeBtn) {
        toggleIncludeBtn.addEventListener('click', async () => {
            ...
            try {
                const response = await fetch(`/writer/api/project/${config.projectId}/section/${encodeURIComponent(currentSection)}/toggle-exclude/`, {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json',
                        'X-CSRFToken': getCsrfToken()
    ...
    if (moveUpBtn) {
        moveUpBtn.addEventListener('click', async () => {
            ...
            try {
                const response = await fetch(`/writer/api/project/${config.projectId}/section/${encodeURIComponent(currentSection)}/move-up/`, {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json',
                        'X-CSRFToken': getCsrfToken()
    ...
    if (moveDownBtn) {
        moveDownBtn.addEventListener('click', async () => {
            ...
            try {
                const response = await fetch(`/writer/api/project/${config.projectId}/section/${encodeURIComponent(currentSection)}/move-down/`, {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json',
                        'X-CSRFToken': getCsrfToken()
