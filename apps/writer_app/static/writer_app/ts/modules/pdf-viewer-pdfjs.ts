...
export interface PDFViewerOptions {
  ...
  renderQuality?: number; // Quality multiplier: 1.0 = standard, 2.0 = 150 DPI, 4.0 = 300 DPI (default)
}
...
export class PDFJSViewer {
  ...
  constructor(options: PDFViewerOptions) {
    ...
    this.renderQuality = options.renderQuality ?? 4.0; // Default 4x for 300 DPI rendering
  ...
  private async renderPage(
    ...
    try {
      ...
      // Create viewport for display size (what user sees)
      ...
      // Set canvas CSS size to display viewport (scales down the high-res rendering)
      canvas.style.width = displayViewport.width + "px";
      canvas.style.height = displayViewport.height + "px";
      canvas.style.display = "block";

      // Disable image smoothing for crisp text rendering
      // When rendering at high resolution and scaling down via CSS, we want pixel-perfect rendering
      context.imageSmoothingEnabled = false;

      pageContainer.appendChild(canvas);
      container.appendChild(pageContainer);

      // Render PDF page to canvas using the high-resolution viewport
      const renderContext = {
      ...
      // Render text layer for text selection (use display viewport for proper positioning)
      await this.renderTextLayer(page, pageContainer, displayViewport, this.currentScale);
      ...
      // await this.renderAnnotations(page, pageContainer, displayViewport);

      console.log("[PDFJSViewer] Rendered page", pageNum, "at", Math.round(72 * outputScale), "DPI");
    } catch (error) {
