...
from ..models import SearchIndex, SearchQuery, SearchResult, UserPreference
...
def track_search_query(request, query_text: str, search_type: str, result_count: int,
                       execution_time: float, filters: Dict = None):
    """
...
def api_scitex_search(request):
    ...
    if not pipeline:
        return JsonResponse({
            'error': 'Failed to initialize search pipeline',
            'results': []
        }, status=500)

    # Execute search
    try:
        logger.info(f"SciTeX search: query='{query[:50]}...', fields={search_fields}, filters={filters}")
        ...
        for scitex_paper in scitex_result.get('results', []):
            # Store in database
            try:
                django_paper = scitex_to_django_paper(scitex_paper, request.user if request.user.is_authenticated else None)
        ...
        logger.info(f"SciTeX search completed: {len(django_results)} results in {execution_time:.2f}s")
...
def api_scitex_search_single(request):
    ...
    try:
        logger.info(f"SciTeX single search: query='{query[:50]}...', filters={filters}, max_results={max_results}")
        ...
        logger.info(f"SciTeX single search completed: {len(django_results)} results in {execution_time:.2f}s")
