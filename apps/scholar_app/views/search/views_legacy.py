...
__FILE__ = "./apps/scholar_app/simple_views.py"
...
def export_bibtex(request):
    ...
    try:
        ...
        papers = (
            SearchIndex.objects.filter(id__in=paper_ids)
            .prefetch_related("authors", "journal")
        ...
        return JsonResponse(
            {
                ...
                "filename": f"scitex_export_{collection_name}_{papers.count()}_papers.bib"
                if collection_name
                else f"scitex_export_{papers.count()}_papers.bib",
                "count": papers.count(),
            }
        )
        ...
        return JsonResponse(
            {
                "success": True,
                "content": ris_content,
                "filename": f"scitex_export_{collection_name}_{papers.count()}_papers.ris"
                if collection_name
                else f"scitex_export_{papers.count()}_papers.ris",
                "count": papers.count(),
            }
        )
        ...
        return JsonResponse(
            {
                ...
                "filename": f"scitex_export_{collection_name}_{papers.count()}_papers.enw"
                if collection_name
                else f"scitex_export_{papers.count()}_papers.enw",
                "count": papers.count(),
            }
        )
        ...
        return JsonResponse(
            {
                "success": True,
                "content": csv_content,
                "filename": f"scitex_export_{collection_name}_{papers.count()}_papers.csv"
                if collection_name
                else f"scitex_export_{papers.count()}_papers.csv",
                "count": papers.count(),
            }
        )
...
def save_paper(request):
    ...
    try:
        ...
        existing = UserLibrary.objects.filter(user=request.user, paper=paper).first()

        if existing:
            return JsonResponse(
                {"status": "info", "message": "Paper already in your library"}
            )
        ...
        return JsonResponse(
            {"status": "success", "message": f"Paper saved to {project_name}"}
        )
        ...
        return JsonResponse(
            {"status": "error", "message": f"Error saving paper: {str(e)}"}
        )
...
def save_papers_bulk(request):
    ...
    try:
        ...
        if not papers:
            return JsonResponse({"status": "error", "message": "No papers provided"})
        ...
        messages.success(
            request, f"✓ Successfully saved {saved_count} paper(s) to {project_name}"
        )
        ...
        return JsonResponse(
            {"status": "error", "message": f"Error saving papers: {str(e)}"}
        )
...
def upload_file(request):
    ...
    try:
        ...
        elif file_type == "bibtex" and not uploaded_file.name.lower().endswith(
            (".bib", ".bibtex")
        ):
...
def mock_save_paper(request):
    ...
    return (
        save_paper(request)
        if request.user.is_authenticated
        else JsonResponse({"status": "info", "message": "Please log in to save papers"})
    )
...
def save_search(request):
    ...
    try:
        ...
        if not name:
            return JsonResponse(
                {"status": "error", "message": "Search name is required"}, status=400
            )
            ...
            searches_data.append(
                {
                    ...
                    "last_run": search.last_run.isoformat()
                    if search.last_run
                    else None,
                }
            )

        return JsonResponse({"status": "success", "saved_searches": searches_data})
...
def delete_saved_search(request, search_id):
    ...
    try:
        ...
        return JsonResponse(
            {"status": "success", "message": "Saved search deleted successfully"}
        )

    except SavedSearch.DoesNotExist:
        return JsonResponse(
            {"status": "error", "message": "Saved search not found"}, status=404
        )
    except Exception as e:
...
def api_library_papers(request):
    ...
    try:
            ...
            papers_data.append(
                {
                    ...
                    "collections": [
                        ...
                        for c in library_paper.collections.all()
                    ],
                    "saved_at": library_paper.saved_at.isoformat(),
                    "pdf_url": paper.pdf_url,
                    "personal_pdf": library_paper.personal_pdf.url
        ...
        return JsonResponse(
            {"status": "success", "papers": papers_data, "count": len(papers_data)}
        )
...
def api_create_collection(request):
    ...
    try:
        ...
        if not name:
            return JsonResponse(
                {"status": "error", "message": "Collection name is required"},
                status=400,
            )
...
def api_update_library_paper(request, paper_id):
    ...
    try:
        ...
        library_paper = UserLibrary.objects.get(user=request.user, paper__id=paper_id)
        ...
        if "importance_rating" in data:
            ...
            if rating is not None and (rating < 1 or rating > 5):
                return JsonResponse(
                    {"status": "error", "message": "Rating must be between 1-5"},
                    status=400,
                )
        ...
        if "collection_ids" in data:
            ...
            collections = Collection.objects.filter(
                user=request.user, id__in=collection_ids
            )
            library_paper.collections.set(collections)

        return JsonResponse(
            {"status": "success", "message": "Paper updated successfully"}
        )

    except UserLibrary.DoesNotExist:
        return JsonResponse(
            {"status": "error", "message": "Paper not found in library"}, status=404
        )
    except json.JSONDecodeError:
        return JsonResponse(
            {"status": "error", "message": "Invalid JSON data"}, status=400
...
def api_remove_library_paper(request, paper_id):
    ...
    try:
        ...
        return JsonResponse(
            {"status": "error", "message": "Paper not found in your library"},
            status=404,
        )
    except Exception as e:
...
def research_trends(request):
    ...
    context = {
        ...
        "active_journals": Journal.objects.filter(searchindex__isnull=False)
        .distinct()
        .count(),
...
def api_trending_papers(request):
    ...
    try:
        ...
        trending_papers = (
            SearchIndex.objects.filter(status="active", created_at__gte=since)
            .select_related("journal")
            .prefetch_related("authors")
            .annotate(total_citations=Count("citations_received"))
            ...
            papers_data.append(
                {
                    ...
                    "is_open_access": paper.is_open_access,
                    "pdf_url": paper.pdf_url,
                    "trend_score": paper.citation_count
                    + (paper.view_count * 0.1),  # Weighted trending score
                }
            )
...
def api_trending_topics(request):
    ...
    try:
        ...
        for paper in recent_papers:
            if paper.keywords:
                keywords = [
                    k.strip().lower() for k in paper.keywords.split(",") if k.strip()
                ]
...
def api_trending_authors(request):
    ...
    try:
        ...
        trending_authors = (
            ...
            .annotate(
                ...
                total_citations=Count(
                    "authorpaper__paper__citations_received", distinct=True
                ),
                avg_citations=Avg("authorpaper__paper__citation_count"),
            )
            .filter(recent_papers__gt=0)
            .order_by("-recent_papers", "-total_citations")[:15]
        )
        ...
        for author in trending_authors:
            ...
            recent_papers = SearchIndex.objects.filter(
                status="active", authors=author, created_at__gte=since
            ...
            authors_data.append(
                {
                    ...
                    "avg_citations": float(author.avg_citations)
                    if author.avg_citations
                    else 0,
...
def api_research_analytics(request):
    ...
    try:
        ...
        total_papers = SearchIndex.objects.filter(status="active").count()
        recent_papers = SearchIndex.objects.filter(
            status="active", created_at__gte=last_month
        ).count()

        # Publication trends by year
        yearly_stats = (
            SearchIndex.objects.filter(status="active", publication_date__isnull=False)
        ...
        top_journals = (
            Journal.objects.annotate(paper_count=Count("searchindex"))
            .filter(paper_count__gt=0)
            .order_by("-paper_count")[:10]
        )

        # Open access statistics
        open_access_count = SearchIndex.objects.filter(
            status="active", is_open_access=True
        ).count()
        open_access_percentage = (
            (open_access_count / total_papers * 100) if total_papers > 0 else 0
        )
        ...
        doc_type_stats = (
            SearchIndex.objects.filter(status="active")
        ...
        analytics_data = {
            "overview": {
                ...
                "growth_rate": (
                    (recent_papers / total_papers * 100) if total_papers > 0 else 0
                ),
                "total_authors": Author.objects.count(),
            ...
            "publication_trends": [
                {"year": int(stat["year"]), "papers": stat["paper_count"]}
            ...
            "document_types": [
                {
                    ...
                    "count": stat["count"],
                    "percentage": round(stat["count"] / total_papers * 100, 1)
                    if total_papers > 0
                    else 0,
                }
...
def paper_annotations(request, paper_id):
    ...
    try:
        ...
        user_annotations = (
            Annotation.objects.filter(paper=paper, user=request.user)
            .prefetch_related("tags", "replies__user")
            .order_by("-created_at")
        )

        # Get public/shared annotations
        public_annotations = (
            Annotation.objects.filter(paper=paper, privacy_level="public")
            .exclude(user=request.user)
            .prefetch_related("tags", "replies__user", "user")
            .order_by("-upvotes", "-created_at")[:10]
        )

        # Get collaboration groups user belongs to
        user_groups = CollaborationGroup.objects.filter(members=request.user).order_by(
            "name"
        )
        ...
        return render(
            ...
            {"error": "Unable to load annotations"},
            status=500,
        )
        ...
        if "tags" in data and data["tags"]:
            for tag_name in data["tags"]:
                tag, created = AnnotationTag.objects.get_or_create(
                    name=tag_name.strip().lower(), defaults={"created_by": request.user}
                )
                if not created:
                    tag.usage_count += 1
                    tag.save()
...
def api_update_annotation(request, annotation_id):
    ...
    try:
        ...
        return JsonResponse(
            {"status": "success", "message": "Annotation updated successfully"}
        )
...
def api_vote_annotation(request, annotation_id):
    ...
    try:
        ...
        if vote_type not in ["up", "down"]:
            return JsonResponse(
                {"status": "error", "message": "Invalid vote type"}, status=400
            )

        # Check if user already voted
        existing_vote = AnnotationVote.objects.filter(
            user=request.user, annotation=annotation
        ).first()
        ...
        if existing_vote:
            ...
            AnnotationVote.objects.create(
                user=request.user, annotation=annotation, vote_type=vote_type
            )
            if vote_type == "up":
                annotation.upvotes += 1
            else:
...
def api_collaboration_groups(request):
    ...
    try:
        groups = (
            CollaborationGroup.objects.filter(members=request.user)
...
def export_bulk_citations(request):
    ...
    try:
        ...
        library_papers = (
            UserLibrary.objects.filter(user=request.user, paper__id__in=paper_ids)
            .select_related("paper", "paper__journal")
        ...
        if not papers:
            return JsonResponse(
                {"error": "No papers found in your library"}, status=404
            )
...
def export_collection(request, collection_id):
    ...
    try:
        ...
        library_papers = (
            UserLibrary.objects.filter(user=request.user, collections=collection)
            .select_related("paper", "paper__journal")
            .prefetch_related("paper__authors")
        )
        ...
        if not papers:
            return JsonResponse(
                {"error": "No papers found in this collection"}, status=404
            )
