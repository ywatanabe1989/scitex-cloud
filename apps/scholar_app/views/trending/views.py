...
from ...models import (
    SearchIndex as Paper, Author, Journal, Citation,
    UserLibrary, Topic
)
...
def api_trending_papers(request):
    ...
    try:
        ...
        trending_papers = Paper.objects.filter(
...
def api_trending_topics(request):
    ...
    try:
        ...
        trending_topics = Topic.objects.filter(
            papers__created_at__gte=cutoff_date
        ).annotate(
...
def api_research_analytics(request):
    ...
    try:
        ...
        # Average metrics
        avg_citations = Paper.objects.aggregate(
            avg=Avg('citation_count')
        )['avg'] or 0
        
        avg_impact_factor = Journal.objects.aggregate(
        ...
        papers_per_year = Paper.objects.filter(
            publication_date__year__gte=timezone.now().year - 10
        ).extra(
            select={'year': 'YEAR(publication_date)'}
