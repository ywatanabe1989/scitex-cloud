...
class DataCiteMetadataBuilder:
    ...
    def build_dataset_metadata(self, dataset: Dataset) -> str:
        ...
        if dataset.repository_doi:
            ...
            identifier.text = dataset.repository_doi.replace('https://doi.org/', '').replace('doi:', '')
    ...
    def build_paper_metadata(self, paper: SearchIndex) -> str:
        ...
        if paper.doi:
            identifier = ET.SubElement(root, 'identifier')
            identifier.set('identifierType', 'DOI')
            identifier.text = paper.doi.replace('https://doi.org/', '').replace('doi:', '')
        
        # Creators
        creators = ET.SubElement(root, 'creators')
        
        # Get authors through AuthorPaper relationship
        author_papers = paper.authors.through.objects.filter(paper=paper).order_by('author_order')
...
class DOIManager:
    ...
    def assign_doi_to_dataset(self, dataset: Dataset, publish: bool = False) -> Optional[str]:
        ...
        try:
            ...
            if dataset.repository_id:
                ...
                repo_data = service.update_dataset(dataset.repository_id, {
                    'title': dataset.title,
    ...
    def publish_dataset_doi(self, dataset: Dataset) -> bool:
        ...
        try:
            if not dataset.repository_id:
                raise DOIAssignmentError("Dataset must be uploaded to repository before publishing DOI")
            ...
            logger.info(f"Published DOI for dataset {dataset.id}: {dataset.repository_doi}")
            return True
    ...
    def update_dataset_metadata(self, dataset: Dataset) -> bool:
        ...
        try:
            if not dataset.repository_id:
                logger.warning(f"Dataset {dataset.id} has no repository ID, cannot update metadata")
                return False
            ...
            # Update dataset
            service.update_dataset(dataset.repository_id, {
                'title': dataset.title,
                'description': dataset.description,
                'metadata_xml': metadata_xml
            })
    ...
    def validate_doi_metadata(self, dataset: Dataset) -> Dict[str, Any]:
        ...
        if dataset.title and len(dataset.title) < 10:
            validation_results['warnings'].append("Title is very short (less than 10 characters)")
        
        if dataset.description and len(dataset.description) < 50:
            validation_results['warnings'].append("Description is very short (less than 50 characters)")
        
        if not dataset.keywords:
            validation_results['warnings'].append("No keywords provided - this helps with discoverability")
        
        if not dataset.license:
            validation_results['warnings'].append("No license specified - consider adding an open license")
        
        if dataset.file_count == 0:
            validation_results['warnings'].append("No files in dataset - add data files before publishing")
        
        # Check ORCID IDs for authors
        if not hasattr(dataset.owner, 'orcid_id') or not dataset.owner.orcid_id:
            validation_results['warnings'].append("Owner has no ORCID ID - this improves author identification")
...
class CitationFormatter:
    ...
    def format_paper_citation(self, paper: SearchIndex, style: str = 'apa') -> str:
        ...
        citation_data = {
            'authors': authors,
            'year': paper.publication_date.year if paper.publication_date else paper.created_at.year,
            'title': paper.title,
            'journal': paper.journal.name if paper.journal else paper.get_source_display(),
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            return {
                ...
                'published_date': data['message'].get('published-print', {}).get('date-parts', [[None]])[0][0],
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            return {
                ...
                'description': attributes.get('descriptions', [{}])[0].get('description'),
            }
