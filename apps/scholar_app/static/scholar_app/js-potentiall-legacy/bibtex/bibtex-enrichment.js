...
function handleJobComplete(data) {
    ...
    if (runningIndicator) {
        console.log('Hiding enrichment running indicator - job status:', data.status);
...
function displayRecentJobs(jobs) {
    ...
    jobs.forEach((job, index) => {
        ...
        const badgeColor = job.status === 'completed' ? 'var(--success-color)' :
                          job.status === 'processing' ? 'var(--scitex-color-03)' :
                          job.status === 'pending' ? 'var(--warning-color)' :
                          job.status === 'failed' ? 'var(--error-color)' :
                          job.status === 'cancelled' ? 'var(--error-color)' :
                          'var(--color-fg-muted)';

        const badgeTextColor = job.status === 'failed' || job.status === 'cancelled' ? 'white' : 'white';
...
window.saveJobToProject = function(jobId) {
    ...
    if (!projectSelector) {
        if (confirm(
            ...
            window.location.href = '/new/?next=' + encodeURIComponent('/scholar/#bibtex');
        }
        return;
    }
