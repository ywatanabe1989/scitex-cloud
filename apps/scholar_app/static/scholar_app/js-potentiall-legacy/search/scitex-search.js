  ...
  const ScitexSearch = {
    ...
    init: function () {
      ...
      this.checkAvailability().then((available) => {
        if (available) {
          ...
          console.warn(
            "[SciTeX Search] Not available, falling back to default search",
          );
        }
      });
    },
    ...
    cacheResults: function (query, data) {
      try {
        ...
        console.log(
          "[SciTeX Search] Results cached to sessionStorage and localStorage",
        );
      } catch (error) {
    ...
    loadCachedResults: function () {
      try {
        ...
        if (cached) {
          ...
          if (cacheAge < maxAge) {
            console.log(
              "[SciTeX Search] Restoring cached results from " +
                source +
                " for query:",
              cacheData.query,
            );
    ...
    displayCachedResults: function (data, searchTime) {
      ...
      if (window.SwarmPlots && results.length > 0) {
        ...
        setTimeout(() => {
          ...
          console.log(
            "[SciTeX Search] Swarm plots initialized with",
            results.length,
            "papers",
          );
        }, 100);
    ...
    executeSearch: async function (query, params) {
      ...
      try {
        ...
        console.log(
          `[SciTeX Search] Found ${data.results.length} results in ${searchTime}s`,
        );
      } catch (error) {
    ...
    showLoadingState: function (query) {
      ...
      if (!resultsContainer) {
        ...
        if (
          progressiveLoadingStatus &&
          progressiveLoadingStatus.parentElement
        ) {
          progressiveLoadingStatus.parentElement.insertBefore(
            resultsContainer,
            progressiveLoadingStatus.nextSibling,
          );
        }
      }
    ...
    updateEngineStatus: function (engineName, status, count) {
      ...
      if (!progressSource) {
        console.warn(
          `[SciTeX Search] Progress element not found for ${engineName}`,
        );
        return;
      }
    ...
    renderPaperCard: function (paper, index) {
      ...
      const paperId =
        paper.id ||
        `paper-${paper.doi || paper.title}`.replace(/[^a-z0-9]/gi, "_");
    ...
    injectSearchModeToggle: function () {
      ...
      if (searchButton && searchButton.parentElement) {
        ...
        // Add click handler
        document
          .getElementById("scitexModeToggle")
...
function saveToLibrary(button) {
  ...
  if (!isAuthenticated) {
    if (
      confirm(
        "To save papers to a project, you need to sign in.\n\nWould you like to sign in now?",
      )
    ) {
  ...
  const paperData = {
    ...
    title:
      card.dataset.title || card.querySelector(".paper-title")?.textContent,
    authors:
      card.dataset.authors || card.querySelector(".paper-authors")?.textContent,
    year: card.dataset.year || card.querySelector(".paper-year")?.textContent,
    doi: card.dataset.doi,
    abstract:
      card.dataset.abstract ||
      card.querySelector(".abstract-preview")?.textContent,
    citations: card.dataset.citations,
    journal:
      card.dataset.journal || card.querySelector(".paper-journal")?.textContent,
    ...
    if (card) {
      papers.push({
        ...
        authors:
          ...
          card.querySelector(".paper-authors")?.textContent,
        year:
          card.dataset.year || card.querySelector(".paper-year")?.textContent,
        ...
        journal:
          ...
          card.querySelector(".paper-journal")?.textContent,
        url: card.dataset.url,
        source: card.dataset.source,
      });
    }
...
function setupGlobalAbstractToggle() {
  ...
  const truncatedBtn = document.querySelector(
    '.global-abstract-toggle[data-mode="truncated"]',
  );
  if (truncatedBtn) {
    truncatedBtn.click();
  }
...
function handleSortChange(sortBy) {
  ...
  cards.sort((a, b) => {
    switch (sortBy) {
      ...
      case "citations":
        return (
          parseInt(b.dataset.citations || 0) -
          parseInt(a.dataset.citations || 0)
        );
      case "impact_factor":
        return (
          parseFloat(b.dataset.impactFactor || 0) -
          parseFloat(a.dataset.impactFactor || 0)
        );
      case "relevance":
      default:
  ...
  const insertPoint = selectionToolbar
    ? selectionToolbar.nextElementSibling
  ...
  const observer = new MutationObserver(function (mutations) {
    ...
    mutations.forEach(function (mutation) {
      if (mutation.type === "childList" && mutation.addedNodes.length > 0) {
        ...
        const hasCards = Array.from(mutation.addedNodes).some(
          (node) => node.classList && node.classList.contains("result-card"),
        );
...
function createProjectSelectorModal() {
  ...
  modal.style.cssText =
    "display: none; position: fixed; top: 0; left: 0; width: 100%; height: 100%; background: rgba(0, 0, 0, 0.5); z-index: 10000; align-items: center; justify-content: center; opacity: 1; pointer-events: auto;";
...
function createBulkProjectSelectorModal() {
  ...
  modal.style.cssText =
    "display: none; position: fixed; top: 0; left: 0; width: 100%; height: 100%; background: rgba(0, 0, 0, 0.5); z-index: 10000; align-items: center; justify-content: center;";
...
window.confirmBulkProjectSave = async function () {
  ...
  try {
    ...
    if (result.success) {
      ...
      document
        .querySelectorAll(".paper-select-checkbox:checked")
        .forEach((cb) => {
          cb.checked = false;
        });
