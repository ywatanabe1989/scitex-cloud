...
(function() {
    ...
    function initializeDualRangeSlider(config) {
        const { minSlider, maxSlider, minInput, maxInput, highlight, min, max, swarmPlotId, field } = config;
    ...
    function updateSwarmPlotSelection(swarmPlotId, field, minVal, maxVal) {
        ...
        circles.forEach(circle => {
            ...
            const card = document.querySelector(`.result-card[data-paper-id="${paperId}"]`);
            if (!card) return;
    ...
    function generateHistograms() {
        ...
        if (papers.length > 0) {
            ...
            const citations = papers.filter(p => p.citations !== null).map(p => p.citations);
            ...
            // Swarm plots
            renderSwarmPlot('yearSwarmPlot', papers.filter(p => p.year), 'year', 1900, 2025);
            renderSwarmPlot('citationsSwarmPlot', papers.filter(p => p.citations !== null), 'citations', 0, 12000);
        }
    }
    ...
    function renderHistogram(containerId, data, numBins, minVal, maxVal) {
        ...
        bins.forEach((count, index) => {
            ...
            bar.style.background = count > 0 ? 'var(--scitex-color-05)' : 'var(--color-border-default)';
            ...
            bar.addEventListener('mouseleave', function() {
                ...
                this.style.background = count > 0 ? 'var(--scitex-color-05)' : 'var(--color-border-default)';
            });
    ...
    function renderSwarmPlot(containerId, papers, field, minVal, maxVal) {
        ...
        positions.forEach(({ x, y, paper }) => {
            ...
            // Tooltip
            const titleText = paper.card ? paper.card.querySelector('.result-title')?.textContent.trim() : '';
            ...
            positions.forEach(({ x, paper }) => {
                if (x >= minX && x <= maxX) {
                    ...
                    if (checkbox) {
                        ...
                        const circle = svg.querySelector(`circle[data-paper-id="${paper.id}"]`);
                        if (circle) {
                            circle.setAttribute('fill', 'var(--scitex-color-03)');
