...
const SwarmPlots = {
  ...
  createYearSwarmPlot: function () {
    ...
    const colors = this.data.map((_, i) =>
      this.filteredIndices.has(i)
        ? this.config.includedColor
        : this.config.filteredColor,
    );

    const hoverText = this.data.map(
      (p) =>
  ...
  createCitationsSwarmPlot: function () {
    ...
    const citationsPadding = Math.max(
      10,
      Math.ceil((maxCitations - minCitations) * 0.05),
    ); // 5% padding, min 10
  ...
  createImpactFactorSwarmPlot: function () {
    const impactFactors = this.data.map((p) =>
      p.impact_factor && p.impact_factor > 0 ? p.impact_factor : null,
    );
    ...
    if (validData.length === 0) {
      ...
      Plotly.newPlot("impactFactorSwarmPlot", [], layout, {
        displayModeBar: false,
      });
      return;
    }
    ...
    const ifRange = [Math.max(0, minIF - ifPadding), maxIF + ifPadding];

    const colors = validData.map((d) =>
      this.filteredIndices.has(d.index)
        ? this.config.includedColor
        : this.config.filteredColor,
    );

    const hoverText = validData.map(
      (d) =>
  ...
  updateSwarmPlots: function () {
    ...
    if (document.getElementById("impactFactorSwarmPlot")) {
      ...
      if (impactColors.length > 0) {
        Plotly.restyle("impactFactorSwarmPlot", {
          "marker.color": [impactColors],
        });
      }
    }
  ...
  attachFilterListeners: function () {
    ...
    if (impactFactorSlider && impactFactorSlider.noUiSlider) {
      impactFactorSlider.noUiSlider.on("update", (values) => {
        this.filters.impactFactorRange = [
          parseFloat(values[0]),
          parseFloat(values[1]),
        ];
        this.updateSwarmPlots();
  ...
  triggerFilteredSearch: function () {
    ...
    if (citationsMinInput)
      citationsMinInput.value = this.filters.citationsRange[0];
    if (citationsMaxInput)
      citationsMaxInput.value = this.filters.citationsRange[1];
    if (impactFactorMinInput)
      impactFactorMinInput.value = this.filters.impactFactorRange[0].toFixed(1);
    if (impactFactorMaxInput)
      impactFactorMaxInput.value = this.filters.impactFactorRange[1].toFixed(1);
   ...
   */
  clear: function () {
    ["yearSwarmPlot", "citationsSwarmPlot", "impactFactorSwarmPlot"].forEach(
      (id) => {
