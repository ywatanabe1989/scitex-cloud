...
function pollBibtexJobStatus(jobId: string, attempts: number = 0): void {
  ...
  if (attempts > 180) {
    ...
    const enrichmentLog = document.getElementById(
      "enrichmentProcessingLog",
    ) as HTMLElement | null;
    if (log)
      log.textContent += "\n\n✗ Polling timeout. Please refresh the page.";
    if (enrichmentLog)
      enrichmentLog.textContent +=
        "\n\n✗ Polling timeout. Please refresh the page.";
    return;
  }
  ...
  fetch(`/scholar/api/bibtex/job/${jobId}/status/`)
    ...
    .then((data: JobStatusResponse) => {
      ...
      if (data.log) {
        const processingLog = document.getElementById(
          "processingLog",
        ) as HTMLElement | null;
        const enrichmentLog = document.getElementById(
          "enrichmentProcessingLog",
      ...
      if (data.status === "completed") {
        ...
        const viewChangesBtn = document.getElementById(
          "viewChangesBtn",
        ) as HTMLButtonElement | null;
        if (viewChangesBtn) {
        ...
        const showDiffBtn = document.getElementById(
          "showDiffBtn",
        ) as HTMLButtonElement | null;
        const openUrlsBtn = document.getElementById(
          "openUrlsBtn",
        ) as HTMLButtonElement | null;
        const openUrlsMainBtn = document.getElementById(
          "openUrlsMainBtn",
        ) as HTMLButtonElement | null;
        ...
        fetch(`/scholar/api/bibtex/job/${jobId}/urls/`)
          ...
          .then((urlData: any) => {
            ...
            const urlCount = document.getElementById(
              "urlCount",
            ) as HTMLElement | null;
            if (urlCount) urlCount.textContent = count.toString();

            // Update main button text
            const urlButtonText = document.getElementById(
              "urlButtonText",
            ) as HTMLElement | null;
    ...
    .then((data: any) => {
      ...
      if (data.urls && data.urls.length > 0) {
        showConfirm({
          ...
          onConfirm: () => {
            console.log(
              `[Open URLs] Opening ${data.urls.length} URLs as tabs...`,
            );
            ...
            if (openedCount === data.urls.length) {
              ...
              console.log(
                `[Open URLs] Successfully opened all ${openedCount} URLs`,
              );
  ...
  if (stats) {
    ...
    if (stats.major_fields) {
      ...
      const abstractRate = stats.major_fields.abstract
        ? Math.round(
            (stats.major_fields.abstract.acquired / stats.total_entries) * 100,
          )
        : 0;
      const abstractColor =
        abstractRate >= 70
      ...
      const doiRate = stats.major_fields.doi
        ? Math.round(
            (stats.major_fields.doi.acquired / stats.total_entries) * 100,
          )
        : 0;
      const doiColor =
        doiRate >= 70 ? "#28a745" : doiRate >= 40 ? "#e67700" : "#d73a49";
      html += `<div style="text-align: center; padding: 0.75rem; background: white; border-radius: 6px; border: 1px solid #e1e4e8;">
      ...
      const citationsRate = stats.major_fields.citation_count
        ? Math.round(
            (stats.major_fields.citation_count.acquired / stats.total_entries) *
              100,
          )
        : 0;
      const citationsColor =
        citationsRate >= 70
          ? "#28a745"
          : citationsRate >= 40
            ? "#e67700"
      ...
      const ifRate = stats.major_fields.impact_factor
        ? Math.round(
            (stats.major_fields.impact_factor.acquired / stats.total_entries) *
              100,
          )
        : 0;
      const ifColor =
        ifRate >= 70 ? "#28a745" : ifRate >= 40 ? "#e67700" : "#d73a49";
      html += `<div style="text-align: center; padding: 0.75rem; background: white; border-radius: 6px; border: 1px solid #e1e4e8;">
 ...
 */
function getStatusBadgeData(status: string): {
  text: string;
  bgColor: string;
  textColor: string;
} {
  const badges: {
    [key: string]: { text: string; bgColor: string; textColor: string };
  } = {
    completed: {
      text: "Completed",
 ...
 */
(window as any).saveJobToProject = async function (
  jobId: string,
): Promise<void> {
