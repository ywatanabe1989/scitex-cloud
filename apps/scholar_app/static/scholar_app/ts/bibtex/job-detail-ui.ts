 ...
 */
async function copyLogToClipboard(button: HTMLButtonElement, logElement: HTMLPreElement): Promise<void> {
 ...
 * @param {HTMLPreElement} logElement - The log content element
 */
function handleLogKeyboardShortcut(event: KeyboardEvent, logElement: HTMLPreElement): void {
 ...
 */
function updateElapsedTime(startedAt: Date | null, elapsedTimeEl: HTMLElement): void {
...
function pollJobStatus(
    ...
    fetch(`/scholar/api/bibtex/job/${jobId}/status/`)
        ...
        .then(data => {
            ...
            // Update progress
            const progressBar = document.querySelector('[style*="background: var(--scitex-color-03)"]') as HTMLElement;
...
function initJobDetailUI(): void {
    ...
    const elements: JobDetailElements = {
        ...
        toggleLogSizeBtn: document.getElementById('toggle-log-size') as HTMLButtonElement,
