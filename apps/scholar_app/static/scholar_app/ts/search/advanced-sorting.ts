...
function initializeAdvancedSorting(): void {
    const advancedSortBtn = document.getElementById('toggleAdvancedSort') as HTMLElement | null;
    if (!advancedSortBtn) return;

    advancedSortBtn.addEventListener('click', function(e: Event) {
        e.preventDefault();
        const container = document.getElementById('advancedSortContainer') as HTMLElement | null;
        if (container) {
    ...
    // Clear all button
    const clearBtn = document.getElementById('clearSortCriteria') as HTMLElement | null;
    if (clearBtn) {
 ...
 */
function addSortCriterionByButton(field: string, order: 'asc' | 'desc', buttonElement: HTMLElement): void {
...
function updateButtonPriorities(): void {
    ...
    sortCriteria.forEach((criterion, index) => {
        const button = document.querySelector(`.sort-criterion-btn[data-field="${criterion.field}"]`) as HTMLElement | null;
        if (button) {
...
function renderSortCriteria(): void {
    const container = document.getElementById('sortCriteriaList') as HTMLElement | null;
    if (!container) return;
...
function updateHiddenSortInput(): void {
    ...
    const sortByInput = document.querySelector('select[name="sort_by"]') as HTMLSelectElement | null;
    ...
    if (sortCriteria.length === 0) {
        ...
        const orderSelect = document.querySelector('select[name="sort_order"]') as HTMLSelectElement | null;
        if (orderSelect) {
...
function initializeResultSelection(): void {
    const selectAllBtn = document.getElementById('selectAllResults') as HTMLElement | null;
    const deselectAllBtn = document.getElementById('deselectAllResults') as HTMLElement | null;
    const exportSelectedBtn = document.getElementById('exportSelectedBibtex') as HTMLElement | null;
...
function updateSelectionCount(): void {
    ...
    const countDisplay = document.getElementById('selectedCount') as HTMLElement | null;
    if (countDisplay) {
        countDisplay.textContent = `${selectedCount} of ${totalCount} selected`;
    }

    const exportBtn = document.getElementById('exportSelectedBibtex') as HTMLButtonElement | null;
    if (exportBtn) {
