...
console.log("[DEBUG] apps/scholar_app/static/scholar_app/ts/search/scitex-search.ts loaded");
declare global {
    interface Window {
        SCHOLAR_CONFIG?: {
            urls?: {
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (!searchForm || !searchInput) {
        console.log('[SciTeX Search] Search form not found, skipping initialization');
        return;
    }
...
function startUnifiedSearch(query: string): void {
    ...
    const selectedCheckboxes = document.querySelectorAll('.source-toggle:checked') as NodeListOf<HTMLInputElement>;
    ...
    const allSources: SourceConfig[] = [
        ...
        { name: 'google_scholar', endpoint: '/scholar/api/search/google-scholar/', maxResults: 50 },
    ...
    const sourcesToSearch = allSources.filter(source => selectedSourceValues.includes(source.name));
    ...
    allSources.forEach(source => {
        if (!selectedSourceValues.includes(source.name)) {
            const progressSource = document.querySelector(`[data-source="${source.name}"]`) as HTMLElement | null;
            if (progressSource) {
                const badge = progressSource.querySelector('.badge') as HTMLElement | null;
                const spinner = progressSource.querySelector('.spinner-border') as HTMLElement | null;
...
function handleSearchError(sourceName: string, errorMessage: string): void {
    const progressSource = document.querySelector(`[data-source="${sourceName}"]`) as HTMLElement | null;
    if (progressSource) {
        const badge = progressSource.querySelector('.badge') as HTMLElement | null;
        const spinner = progressSource.querySelector('.spinner-border') as HTMLElement | null;
        const count = progressSource.querySelector('.count') as HTMLElement | null;
