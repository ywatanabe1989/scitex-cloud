...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const searchForm = document.querySelector('form') as HTMLFormElement | null;
    const searchInput = document.querySelector('input[name="q"]') as HTMLInputElement | null;
    const searchButton = document.getElementById('searchButton') as HTMLButtonElement | null;
    const progressiveLoadingStatus = document.getElementById('progressiveLoadingStatus') as HTMLElement | null;
    const progressiveResults = document.getElementById('progressiveResults') as HTMLElement | null;

    // Advanced filters functionality
    const toggleAdvancedFilters = document.getElementById('toggleAdvancedFilters') as HTMLElement | null;
    const advancedFilters = document.getElementById('advancedFilters') as HTMLElement | null;
    const clearFiltersBtn = document.getElementById('clearFilters') as HTMLElement | null;
    const saveSearchBtn = document.getElementById('saveSearch') as HTMLElement | null;
    ...
    function updateActiveFilterCount(): void {
        ...
        const activeFilterCountEl = document.getElementById('activeFilterCount') as HTMLElement | null;
    ...
    function saveSourcePreferences(): void {
        ...
        if (window.scholarConfig && window.scholarConfig.user && window.scholarConfig.user.isAuthenticated) {
            ...
            .catch((error: Error) => console.error('Error saving preferences:', error));
        } else {
    ...
    function getCsrfToken(): string {
        return (document.querySelector('[name=csrfmiddlewaretoken]') as HTMLInputElement)?.value ||
               (document.querySelector('meta[name="csrf-token"]') as HTMLMetaElement)?.getAttribute('content') ||
               getCookie('csrftoken') || '';
    }
    ...
    function updateSourceDisplay(): void {
        const selectedSources = document.querySelectorAll('.source-toggle:checked') as NodeListOf<HTMLInputElement>;
        const sourceDisplay = document.getElementById('sourceDisplay') as HTMLElement | null;
    ...
    function loadSourcePreferences(): void {
        if (window.scholarConfig && window.scholarConfig.user && window.scholarConfig.user.isAuthenticated) {
    ...
    function loadSourcePreferencesFromStorage(): void {
    ...
    function applySourcePreferences(preferences: { [key: string]: boolean }): void {
    ...
    function setDefaultSourcePreferences(): void {
    ...
    function initializeSourceToggles(): void {
        const allSourcesToggle = document.getElementById('source_all_toggle') as HTMLInputElement | null;
        const sourceToggles = document.querySelectorAll('.source-toggle') as NodeListOf<HTMLInputElement>;
    ...
    function updateAllSourcesToggle(): void {
        const allSourcesToggle = document.getElementById('source_all_toggle') as HTMLInputElement | null;
        const sourceToggles = document.querySelectorAll('.source-toggle') as NodeListOf<HTMLInputElement>;
        const checkedToggles = document.querySelectorAll('.source-toggle:checked') as NodeListOf<HTMLInputElement>;
    ...
    function handleSourceChange(): void {
        ...
        if (searchInput && searchInput.value.trim()) {
            const autoSubmitForm = document.querySelector('form') as HTMLFormElement | null;
            if (autoSubmitForm) {
    ...
    if (!window._scholarSortInitialized) {
        ...
        if (sortSelect && !(sortSelect as any)._listenerAttached) {
            sortSelect.addEventListener('change', function() {
                const currentUrl = new URL(window.location.href);
                currentUrl.searchParams.set('sort_by', this.value);
                window.location.href = currentUrl.toString();
            });
            (sortSelect as any)._listenerAttached = true;
        }
    ...
    document.querySelectorAll('.abstract-toggle').forEach(toggle => {
        toggle.addEventListener('click', function(e: Event) {
            e.preventDefault();
            const abstractPreview = (this as HTMLElement).closest('.abstract-preview');
            if (!abstractPreview) return;

            const shortAbstract = abstractPreview.querySelector('.abstract-short') as HTMLElement | null;
            const fullAbstract = abstractPreview.querySelector('.abstract-full') as HTMLElement | null;
    ...
    const toggleButtons = document.querySelectorAll('.global-abstract-toggle') as NodeListOf<HTMLElement>;
    ...
    function applyGlobalAbstractMode(mode: string): void {
        const abstracts = document.querySelectorAll('.abstract-preview') as NodeListOf<HTMLElement>;
...
(window as any).viewOnWeb = function(link: HTMLElement, event: MouseEvent): boolean {
...
(window as any).showToast = function(message: string, type: string = 'info'): void {
