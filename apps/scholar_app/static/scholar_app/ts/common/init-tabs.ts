...
document.addEventListener('DOMContentLoaded', (): void => {
    ...
    const tabContents: NodeListOf<Element> = document.querySelectorAll('.vertical-tab-content');
    ...
    function switchTab(targetTab: string): void {
        ...
        tabContents.forEach((content: Element): void => content.classList.remove('active'));

        // Add active class to target tab and content
        const targetLink: Element | null = document.querySelector(`.tab-link[data-tab="${targetTab}"]`);
        ...
        if (targetTab === 'search') {
            setTimeout((): void => {
                const searchInput = document.querySelector<HTMLInputElement>('#search-tab input[name="q"]');
                if (searchInput) {
                    searchInput.focus();
    ...
    function initializeTab(): void {
        ...
        const hasSearchQuery: boolean = urlParams.get('q') !== null && urlParams.get('q')!.trim() !== '';
    ...
    const searchInput: HTMLInputElement | null = document.querySelector('#search-tab input[name="q"]');
    const searchForm: HTMLFormElement | null = document.getElementById('literatureSearchForm') as HTMLFormElement | null;
