...
document.addEventListener("DOMContentLoaded", function () {
  ...
  document.querySelectorAll(".abstract-toggle").forEach((toggle) => {
    toggle.addEventListener("click", function (e: Event) {
      e.preventDefault();
      const abstractPreview = (this as HTMLElement).closest(
        ".abstract-preview",
      );
      if (!abstractPreview) return;

      const shortAbstract = abstractPreview.querySelector(
        ".abstract-short",
      ) as HTMLElement | null;
      const fullAbstract = abstractPreview.querySelector(
        ".abstract-full",
      ) as HTMLElement | null;
  ...
  function pollBibtexJobStatus(jobId: string, attempts: number = 0): void {
    ...
    console.log(
      "[Scholar Index Main] pollBibtexJobStatus called with jobId:",
      jobId,
    );
  }
...
(window as any).viewOnWeb = function (
  link: HTMLElement,
  event: MouseEvent,
): boolean {
  if (
