...
def search_dashboard(request):
    ...
    if query:
        ...
        if (
            cached_results and not request.user.is_authenticated
        ):  # Only use cache for anonymous users
    ...
    optimized_results = (
        results.select_related("journal")
        .prefetch_related(
            "authors",
            "topics",
...
def paper_detail(request, paper_id):
    ...
    paper = get_object_or_404(
        SearchIndex.objects.select_related("journal").prefetch_related(
            "authors", "topics", "citations_made", "citations_received"
        ),
        id=paper_id,
        status="active",
    )
    ...
    related_papers = [
        sp[0] for sp in similar_papers_with_scores
...
def author_profile(request, author_id):
    ...
    if cached_data:
        ...
        author = get_object_or_404(
            Author.objects.select_related().only(
                "id",
                "first_name",
                "last_name",
                "email",
        ...
        papers = (
            SearchIndex.objects.filter(authors=author, status="active")
        ...
        # Calculate metrics
        total_citations = (
            papers.aggregate(total=Count("citations_received"))["total"] or 0
        )
        avg_citations = papers.aggregate(avg=Avg("citation_count"))["avg"] or 0

        # Get co-authors with optimized query
        co_authors = (
            Author.objects.filter(authorpaper__paper__in=papers)
            .exclude(id=author.id)
            .annotate(collaboration_count=Count("authorpaper"))
            .only("id", "first_name", "last_name", "affiliation")
...
def recommendations(request):
    ...
    recent_views = RecommendationLog.objects.filter(
        user=request.user, clicked=True
...
def trending_papers(request):
    ...
    papers = SearchIndex.objects.filter(
        status="active", publication_date__gte=date_threshold
    )
    ...
    if metric == "views":
        ...
        papers = papers.extra(
            select={
                "trending_score": "view_count * 0.3 + citation_count * 0.5 + download_count * 0.2"
            }
    ...
    # Get trending topics
    trending_topics = (
        Topic.objects.filter(searchindex__publication_date__gte=date_threshold)
        .annotate(recent_papers=Count("searchindex"))
...
def _apply_search_filters(request, queryset):
    ...
    return {"queryset": queryset, "active_filters": active_filters}
...
def _get_search_suggestions(query):
    ...
    for topic in topics:
        suggestions.append(
            {"type": "topic", "text": topic.name, "count": topic.paper_count}
        )
...
def _get_similar_papers_recommendations(user, recent_views):
    ...
    for paper in viewed_papers[:5]:
        similar_papers = _calculate_paper_similarity(
            paper, exclude_ids=[p.id for p in viewed_papers]
        )
...
def _calculate_paper_similarity(source_paper, exclude_ids=None, limit=10):
    ...
    candidates = (
        ...
        .exclude(id__in=exclude_ids + [source_paper.id])
        .select_related("journal")
...
def _compute_similarity_score(paper1, paper2):
    ...
    if common_topics:
        topic_score = len(common_topics) / max(
            paper1.topics.count(), paper2.topics.count(), 1
        )
    ...
    if common_authors:
        author_score = len(common_authors) / max(
    ...
    if paper1.journal and paper2.journal:
        if paper1.journal == paper2.journal:
            ...
            hasattr(paper1.journal, "category")
            and hasattr(paper2.journal, "category")
            and paper1.journal.category == paper2.journal.category
        ):
            total_score += 0.05
...
def _papers_have_citation_relationship(paper1, paper2):
    ...
    paper1_citations = set(
        Citation.objects.filter(citing_paper=paper1).values_list(
            "cited_paper_id", flat=True
        )
    )
    paper2_citations = set(
        Citation.objects.filter(citing_paper=paper2).values_list(
            "cited_paper_id", flat=True
        )
    )
...
def _get_author_based_recommendations(user, recent_views):
    ...
    authors = (
        Author.objects.filter(searchindex__in=viewed_papers)
        .annotate(view_count=Count("searchindex"))
        .order_by("-view_count")[:10]
    )

    for author in authors:
        papers = (
            SearchIndex.objects.filter(authors=author)
            .exclude(id__in=[p.id for p in viewed_papers])
    ...
    for paper in viewed_papers[:3]:
        ...
        citing_papers = (
            SearchIndex.objects.filter(
                citations_made__cited_paper=paper, citation_count__gte=10
            )
            .exclude(id__in=[p.id for p in viewed_papers])
...
def _get_trending_recommendations(user):
    ...
    trending = (
        ...
        .prefetch_related(
            Prefetch(
                "authors", queryset=Author.objects.only("id", "first_name", "last_name")
            )
        )
        .only(
            "id",
...
def _get_mixed_recommendations(user, recent_views):
    ...
    all_recommendations.extend(
        _get_similar_papers_recommendations(user, recent_views)[:5]
    )
    all_recommendations.extend(
        _get_author_based_recommendations(user, recent_views)[:5]
    )
    all_recommendations.extend(
        _get_citation_based_recommendations(user, recent_views)[:5]
    )
...
def _export_as_csv(papers):
    ...
    # Write header
    writer.writerow(
        ["Title", "Authors", "Journal", "Year", "DOI", "Abstract", "Citations"]
    )
    ...
    response["Content-Disposition"] = (
        f'attachment; filename="scitex_papers_{datetime.now().strftime("%Y%m%d")}.csv"'
    )
    return response
...
def _export_as_bibtex(papers):
    ...
    response["Content-Disposition"] = (
        f'attachment; filename="scitex_papers_{datetime.now().strftime("%Y%m%d")}.bib"'
    )
    return response
...
def _export_as_ris(papers):
    ...
    response = HttpResponse(output.getvalue(), content_type="text/plain")
    response["Content-Disposition"] = (
        f'attachment; filename="scitex_papers_{datetime.now().strftime("%Y%m%d")}.ris"'
    )
    return response
...
def track_click(request):
    ...
    if result_id:
        SearchResult.objects.filter(
            id=result_id, search_query__user=request.user
        ).update(clicked=True, click_timestamp=timezone.now())
...
def recommendation_feedback(request):
    ...
    if recommendation_id and feedback:
        RecommendationLog.objects.filter(
            ...
            feedback=feedback, feedback_text=feedback_text, interacted_at=timezone.now()
        )
