...
def simple_search(request):
    ...
    if query:
        logger.info(
            f"Scholar search: query='{query}', sources='{sources}', filters='{filters}', user='{request.user.username if request.user.is_authenticated else 'anonymous'}'"
        )
            ...
            for search_func, limit in search_functions:
                try:
                    ...
                    source_results = search_func(
                        query, max_results=limit, filters=filters
                    )
                    ...
                    logger.warning(
                        f"Search function {search_func.__name__} failed: {e}"
                    )
                    continue
    elif sources == "arxiv":
                ...
                results.append(
                    {
                        ...
                        "abstract": (summary.text.strip()[:150] + "...")
...
def save_search(request):
    ...
    try:
        ...
        if not name or not query:
            return JsonResponse(
                {"status": "error", "message": "Name and query are required"},
                status=400,
            )
