...
class PullRequest(models.Model):
    ...
    description = models.TextField(
        blank=True,
        help_text="PR description (supports Markdown)"
    )
    ...
    merge_commit_sha = models.CharField(
        max_length=40,
        blank=True,
        help_text="SHA of the merge commit"
    )
    ...
    has_conflicts = models.BooleanField(
        default=False,
        help_text="Whether PR has merge conflicts"
    )
    ...
    required_approvals = models.IntegerField(
        default=0,
        help_text="Number of approvals required before merge"
    )
    ...
    def save(self, *args, **kwargs):
        ...
        if not self.number:
            last_pr = PullRequest.objects.filter(project=self.project).order_by('-number').first()
    ...
    def get_approval_status(self):
        ...
        changes_requested = sum(1 for r in latest_reviews.values() if r.state == 'changes_requested')
        ...
        return {
            ...
            'is_approved': approved >= self.required_approvals and changes_requested == 0,
        }
    ...
    def get_absolute_url(self):
        ...
        return reverse('user_projects:pr_detail', kwargs={
...
class PullRequestReview(models.Model):
    ...
    content = models.TextField(
        blank=True,
        help_text="Review comments (supports Markdown)"
    )
    created_at = models.DateTimeField(auto_now_add=True, db_index=True)
...
class PullRequestCommit(models.Model):
    ...
    author_name = models.CharField(
        max_length=255,
        help_text="Git author name"
    )
    author_email = models.CharField(
        max_length=255,
        help_text="Git author email"
    )
