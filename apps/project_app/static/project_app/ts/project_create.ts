...
const submitButton = document.querySelector('.btn-primary') as HTMLButtonElement | null;
...
(document.getElementById('template_select') as HTMLSelectElement)?.addEventListener('change', function(this: HTMLSelectElement) {
    // Hide all template details
    document.querySelectorAll('.template-details').forEach((el: Element) => (el as HTMLElement).style.display = 'none');
    // Show selected template details
    const selectedId = this.value;
    const detailsEl = document.querySelector(`.template-details[data-template-id="${selectedId}"]`) as HTMLElement;
    if (detailsEl) {
...
(document.getElementById('initialize_template') as HTMLInputElement)?.addEventListener('change', function(this: HTMLInputElement) {
    if (this.checked && document.getElementById('template_select')) {
        const firstTemplateId = (document.getElementById('template_select') as HTMLSelectElement).value;
        const detailsEl = document.querySelector(`.template-details[data-template-id="${firstTemplateId}"]`) as HTMLElement;
        if (detailsEl) {
            detailsEl.style.display = 'block';
        }
    }
...
if (nameInput) {
    ...
    const clearAutofill = function() {
        ...
        if (nameInput && nameInput.value && nameInput.matches(':-webkit-autofill')) {
            nameInput.value = '';
        }
    };
