...
(function() {
    ...
    function highlightActiveOutlineItem(pageNum: number, autoScroll: boolean = true): void {
    ...
    script.onload = function() {
        ...
        pdfjsLib.getDocument(pdfUrl).promise.then(function(pdf: any) {
            pdfDoc = pdf;
            ...
            pdf.getOutline().then(function(outline: any[]) {
                const outlineContainer = document.getElementById('pdf-outline');
                ...
                if (outline && outline.length > 0) {
                    ...
                    const renderOutlineItem = function(item: any, level: number = 1): HTMLElement {
                        ...
                        if (item.dest) {
                            pdf.getDestination(item.dest).then(function(dest: any) {
                                if (dest) {
                                    pdf.getPageIndex(dest[0]).then(function(pageIndex: number) {
