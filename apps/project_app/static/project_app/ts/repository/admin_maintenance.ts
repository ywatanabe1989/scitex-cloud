...
console.log(
  "[DEBUG] /home/ywatanabe/proj/scitex-cloud/apps/project_app/static/project_app/ts/repository/admin_maintenance.ts loaded",
);
...
class RepositoryMaintenance {
    ...
    if (statusEl) {
      ...
      statusEl.querySelectorAll(".health-card").forEach((card) => {
        card.addEventListener("click", () => {
          const filter = card.getAttribute("data-filter") as
            | "all"
            | "healthy"
            | "warnings"
            | "critical";
  ...
  private applyFilter(
    filter: "all" | "healthy" | "warnings" | "critical",
  ): void {
  ...
  private async deleteRepository(repositoryName: string): Promise<void> {
    console.log(
      "[Repository Maintenance] Deleting repository:",
      repositoryName,
    );

    try {
      const response = await fetch(
        `/${this.username}/api/repository-cleanup/`,
        {
          method: "POST",
          headers: {
  ...
  private async restoreRepository(
    ...
    try {
      const response = await fetch(
        `/${this.username}/api/repository-restore/`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
