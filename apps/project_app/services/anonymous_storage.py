...
# -*- coding: utf-8 -*-
"""
Anonymous User Storage Management
...
def get_anonymous_storage_path(session_key: str) -> str:
    """
    Get or create storage path for anonymous session.

    Args:
        session_key: Django session key

    Returns:
        Absolute path to anonymous user's storage directory
    """
    if not session_key:
        raise ValueError("Session key is required for anonymous storage")
...
def save_session_data(session_key: str, data: dict) -> bool:
    """
    Save data to anonymous user's session storage.
...
def load_session_data(session_key: str) -> Optional[dict]:
    """
    Load data from anonymous user's session storage.
    ...
    """
    try:
        storage_path = os.path.join(ANONYMOUS_STORAGE_ROOT, session_key)
        data_path = os.path.join(storage_path, "scholar_data.json")
...
def cleanup_expired_sessions() -> Tuple[int, int]:
    ...
    """
    if not os.path.exists(ANONYMOUS_STORAGE_ROOT):
        logger.info("Anonymous storage root does not exist, skipping cleanup")
        return (0, 0)
    ...
    logger.info(f"Starting cleanup of anonymous sessions in {ANONYMOUS_STORAGE_ROOT}")

    for session_dir in os.listdir(ANONYMOUS_STORAGE_ROOT):
        session_path = os.path.join(ANONYMOUS_STORAGE_ROOT, session_dir)
...
def migrate_to_user_storage(session_key: str, user) -> bool:
    """
    Move anonymous session data to user's permanent storage on signup.
    ...
    """
    anonymous_path = os.path.join(ANONYMOUS_STORAGE_ROOT, session_key)

    if not os.path.exists(anonymous_path):
        logger.info(f"No anonymous data to migrate for session {session_key[:8]}...")
        return False
    ...
    try:
        ...
        # Create a "from_anonymous" project
        ...
        migrated_files = []
        for item in os.listdir(anonymous_path):
            if item == "metadata.json":
                continue

            src = os.path.join(anonymous_path, item)
        ...
        logger.error(f"Failed to migrate anonymous session to user storage: {e}")
        return False


def get_session_info(session_key: str) -> Optional[dict]:
    """
    Get metadata about an anonymous session.
...
def delete_anonymous_session(session_key: str) -> bool:
    """
    Delete an anonymous user's session data.
    ...
    """
    try:
        session_path = os.path.join(ANONYMOUS_STORAGE_ROOT, session_key)
        if os.path.exists(session_path):
            shutil.rmtree(session_path)
            logger.info(f"Deleted anonymous session: {session_key[:8]}...")
            return True
        return False
    except Exception as e:
