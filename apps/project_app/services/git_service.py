...
def configure_git_credentials(project_dir: Path, username: str, token: str):
    ...
    try:
        ...
        if origin_url.startswith('http'):
            ...
            new_url = origin_url.replace(f'http://{gitea_url}', auth_url).replace(f'https://{gitea_url}', auth_url)
...
def auto_commit_file(project_dir: Path, filepath: str, message: str = None) -> Tuple[bool, str]:
    """
