...
def get_current_project(request, user=None):
    ...
    if current_project_slug:
        try:
            ...
            logger.info(f"Using session project for user {user.username}: {current_project.name}")
            return current_project
        except Project.DoesNotExist:
    ...
    try:
        ...
        if current_project:
            logger.info(f"Using first user project for {user.username}: {current_project.name}")
