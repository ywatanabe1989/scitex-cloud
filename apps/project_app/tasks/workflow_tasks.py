...
def execute_workflow_run(self, run_id):
    ...
    try:
        run = WorkflowRun.objects.select_related('workflow', 'workflow__project').get(id=run_id)
...
def execute_workflow_job(self, job_id):
    ...
    try:
        job = WorkflowJob.objects.select_related('run', 'run__workflow', 'run__workflow__project').get(id=job_id)
