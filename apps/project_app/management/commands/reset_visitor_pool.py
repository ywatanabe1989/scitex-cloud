...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        if options['free_expired']:
            ...
            self.stdout.write(self.style.SUCCESS(f'✓ Freed {freed} expired allocations'))
        ...
        if options['visitor']:
            ...
            if visitor_num < 1 or visitor_num > VisitorPool.POOL_SIZE:
                self.stdout.write(self.style.ERROR(f'Error: Visitor number must be 1-{VisitorPool.POOL_SIZE}'))
                return

            self.stdout.write(f'Resetting visitor-{visitor_num:03d}...')
            # TODO: Implement single visitor reset
            self.stdout.write(self.style.WARNING('Single visitor reset not yet implemented'))
            return
