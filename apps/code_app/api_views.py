...
class NotebookDetailAPI(NotebookAPIView):
        ...
        try:
            ...
            return JsonResponse({
                ...
                'notebook': {
                    ...
                    'created_at': notebook.created_at.isoformat(),
                    'updated_at': notebook.updated_at.isoformat(),
                    'shared_with': [user.username for user in notebook.shared_with.all()]
                }
            })
        ...
        try:
            ...
            if title and title != notebook.title:
                ...
                if Notebook.objects.filter(
                    user=request.user, 
                    title=title
                ).exclude(notebook_id=notebook_id).exists():
                    return JsonResponse({
                        'status': 'error',
                        'message': 'A notebook with this title already exists'
                    }, status=400)
    ...
    def delete(self, request, notebook_id):
        ...
        try:
            ...
            if notebook.file_path and notebook.file_path.startswith(str(manager.base_path)):
                try:
                    import os
                    if os.path.exists(notebook.file_path):
                        os.remove(notebook.file_path)
                except Exception as e:
                    logger.warning(f"Could not delete notebook file {notebook.file_path}: {e}")
...
class NotebookConversionAPI(NotebookAPIView):
    ...
    def get(self, request, notebook_id, format_type):
        ...
        try:
            ...
            logger.error(f"Error converting notebook {notebook_id} to {format_type}: {e}")
            return JsonResponse({'status': 'error', 'message': str(e)}, status=500)
