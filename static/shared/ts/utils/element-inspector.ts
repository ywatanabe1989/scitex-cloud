    ...
    allElements.forEach((element: Element) => {
      ...
      box.addEventListener("click", async (e: MouseEvent) => {
        ...
        if (this.currentlyHoveredBox && this.currentlyHoveredElement) {
          ...
          const overlappingBoxes = this.findOverlappingBoxesAtPoint(
            e.clientX,
            e.clientY,
          );
...
`;
  ...
  private updateSelectionHighlights(rect: {
    left: number;
    top: number;
    width: number;
    height: number;
  ...
  private findElementsInRect(rect: {
    left: number;
    top: number;
    width: number;
    height: number;
...
`;
    ...
    elements.slice(0, maxDetailedElements).forEach((element, index) => {
      ...
      const elementDebugInfo = this.gatherElementDebugInfoForSelection(
        element,
        index + 1,
      );
  ...
  private gatherElementDebugInfoForSelection(
    element: Element,
...
## Attributes
${
  Object.entries(info.attributes).length > 0
    ? Object.entries(info.attributes)
        .map(([key, value]) => `- ${key}: ${value}`)
        .join("\n")
    : "No attributes"
...
`;
  ...
  private findOverlappingBoxesAtPoint(
    clientX: number,
    clientY: number,
  ): HTMLDivElement[] {
  ...
  private onDocumentClickForLayerPicker = (e: MouseEvent): void => {
    if (
      this.layerPickerMenu &&
      !this.layerPickerMenu.contains(e.target as Node)
    ) {
      this.closeLayerPicker();
    }
  };
