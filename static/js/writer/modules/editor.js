...
export class WriterEditor {
    constructor(config) {
        ...
        if (window.CodeMirror) {
            ...
            console.warn('[Editor] CodeMirror not found. Editor will not be initialized.');
        }
    }
    /**
    ...
    setupEditor() {
        ...
        // Track undo/redo
        this.editor.on('beforeChange', (_editor, change) => {
     ...
     */
    loadHistory() {
        const stored = this.storage.load('history');
        if (stored) {
            this.history = stored;
