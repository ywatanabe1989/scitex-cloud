/**
 * SciTeX Seekbar Component - JavaScript Enhancement
 *
...
class ScitexSeekbar {
     ...
     *
     * @param {string|HTMLElement} element - Selector or DOM element
     * @param {Object} options - Configuration options
     * @param {number} options.min - Minimum value (default: 0)
     * @param {number} options.max - Maximum value (default: 100)
     * @param {number} options.valueMin - Initial minimum value
     * @param {number} options.valueMax - Initial maximum value
     * @param {number} options.step - Step increment (default: 1)
     * @param {Function} options.format - Value formatting function
     * @param {Function} options.onChange - Called when value changes
     * @param {Function} options.onUpdate - Called during drag
     * @param {Function} options.onStart - Called when drag starts
     * @param {Function} options.onEnd - Called when drag ends
     * @param {boolean} options.showLabels - Show value labels on hover (default: true)
     * @param {boolean} options.showValues - Show value display below (default: false)
     */
    constructor(element, options = {}) {
        ...
        // Default options
        this.options = {
            min: parseFloat(this.container.dataset.min) || 0,
            max: parseFloat(this.container.dataset.max) || 100,
            valueMin: parseFloat(this.container.dataset.valueMin) || 25,
            valueMax: parseFloat(this.container.dataset.valueMax) || 75,
            step: parseFloat(this.container.dataset.step) || 1,
    ...
    _buildDOM() {
        ...
        handleMin.className = 'scitex-seekbar-dual-handle';
        handleMin.setAttribute('role', 'slider');
        handleMin.setAttribute('aria-label', 'Minimum value');
        handleMin.setAttribute('aria-valuemin', this.options.min);
        handleMin.setAttribute('aria-valuemax', this.options.max);
        handleMin.setAttribute('aria-valuenow', this.values.min);
        handleMin.setAttribute('tabindex', '0');
        handleMin.dataset.handle = 'min';

        if (this.options.showLabels) {
            const labelMin = document.createElement('div');
            labelMin.className = 'scitex-seekbar-dual-handle-label';
            labelMin.textContent = this.options.format(this.values.min);
        ...
        handleMax.className = 'scitex-seekbar-dual-handle';
        handleMax.setAttribute('role', 'slider');
        handleMax.setAttribute('aria-label', 'Maximum value');
        handleMax.setAttribute('aria-valuemin', this.options.min);
        handleMax.setAttribute('aria-valuemax', this.options.max);
        handleMax.setAttribute('aria-valuenow', this.values.max);
        handleMax.setAttribute('tabindex', '0');
        handleMax.dataset.handle = 'max';

        if (this.options.showLabels) {
            const labelMax = document.createElement('div');
            labelMax.className = 'scitex-seekbar-dual-handle-label';
            labelMax.textContent = this.options.format(this.values.max);
        ...
        this.elements = {
            ...
            handleMin,
            handleMax,
            labelMin: this.options.showLabels ? handleMin.querySelector('.scitex-seekbar-dual-handle-label') : null,
            labelMax: this.options.showLabels ? handleMax.querySelector('.scitex-seekbar-dual-handle-label') : null
        };
        ...
        if (this.options.showValues) {
            ...
            this.elements.valueMin = valuesDiv.querySelector('[data-value="min"]');
            this.elements.valueMax = valuesDiv.querySelector('[data-value="max"]');
        }
    }
